...
import {
  ...
  Settings, X, Shield, Activity, Car, DollarSign,
  Navigation, Route, Users, MonitorPlay, TriangleAlert,
  Radio, Leaf, Database, Zap, Globe, LogOut,
} from "lucide-react";
...
export function MobileNav({ isOpen, onClose }: MobileNavProps) {
  ...
  return (
    <div className="relative z-50 lg:hidden">
      ...
      {/* Drawer */}
      <div className="fixed inset-y-0 left-0 flex flex-col w-full max-w-xs bg-bg-secondary shadow-xl">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-2 text-text-muted hover:text-white transition-colors rounded-md"
          >
            <X className="h-6 w-6" />
          </button>
        </div>
        ...
        <div className="border-t border-border p-4 flex-shrink-0">
          <div className="flex items-center gap-3 rounded-lg p-2 transition-colors hover:bg-bg-tertiary">
            ...
            <div className="flex flex-1 flex-col truncate">
              ...
              <span className="truncate text-xs text-text-muted">{userRole}</span>
            </div>
            <button
